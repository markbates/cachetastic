...
class Cachetastic::Adapters::Memcache < Cachetastic::Adapters::Base
  ...
  def setup
    self.conn = MemCache.new(configuration.memcache_servers, configuration.store_options.merge({:namespace => self.namespace}))
  ...
  def ns_versions
    ivar_cache do
      ns_conn = MemCache.new(configuration.memcache_servers, configuration.store_options.merge({:namespace => :namespace_versions}))
    end
  end
